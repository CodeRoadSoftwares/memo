...
export function startReminderScheduler() {
  console.log(
    "⏰ [Scheduler] Starting Action Scheduler (runs every minute)...",
  );
  ...
  cron.schedule("* * * * *", async () => {
    try {
      ...
      console.log(
        `⏰ [Scheduler] Found ${actionsDue.length} scheduled action(s) due for execution.`,
      );
      for (const action of actionsDue) {
        let isFinalState = false;
        const payload = (action.payload || {}) as any;
        const originalTime = payload.originalScheduledFor
          ? new Date(payload.originalScheduledFor)
          : action.scheduledFor;

        if (!payload.originalScheduledFor && action.scheduledFor) {
          payload.originalScheduledFor = action.scheduledFor.toISOString();
        }
        ...
        try {
          ...
          if (
            ...
            if (!recipientPhone) {
              console.warn(
                `⏰ [Scheduler] Scheduled message ${action.id} has no recipientPhone in payload, marking as permanently failed.`,
              );
              isFinalState = true;
              await prisma.action.update({
                where: { id: action.id },
              ...
              console.log(
                `⏰ [Scheduler] Sending scheduled message to ${recipientPhone} for User ${action.userId}...`,
              );
              success = await sendMessageToRecipient(
                action.userId,
                recipientPhone,
                messageContent,
              );
              ...
              if (success && targetId) {
                await sendUnifiedMessage(
                  action.platform,
                  targetId,
                  `✅ System has successfully delivered your scheduled message to *${payload.recipientName || recipientPhone}*: "${messageContent}"`,
                );
              }
            }
          } else {
            if (!targetId) {
              console.warn(
                `⏰ [Scheduler] Action ${action.id} has no destination address, permanently failing.`,
              );
              isFinalState = true;
              await prisma.action.update({
                where: { id: action.id },
              ...
              console.log(
                `⏰ [Scheduler] Executing ${action.type}: "${action.title}" for ${targetId} on platform ${action.platform}...`,
              );
              notificationText = `⏰ *Reminder:* ${action.title}`;
              success = await sendUnifiedMessage(
                action.platform,
                targetId,
                notificationText,
              );
          ...
          if (isFinalState) {
            ...
            if (retryCount < 5) {
              console.warn(
                `⏰ [Scheduler] Action ${action.id} failed to send. Retrying in 1 min (Attempt ${retryCount + 1}/5)`,
              );
              ...
              await prisma.action.update({
                ...
                data: {
                  status: "pending",
                  payload,
                  scheduledFor: new Date(Date.now() + 1 * 60 * 1000),
                },
              });
            } else {
              console.error(
                `⏰ [Scheduler] Action ${action.id} permanently failed after 5 retries.`,
              );
              isFinalState = true;
              await prisma.action.update({
                where: { id: action.id },
          ...
          console.error(
            `⏰ [Scheduler] Error processing action ${action.id}:`,
            execErr,
          );
          const retryCount = payload.retryCount || 0;
          if (retryCount < 5) {
            console.warn(
              `⏰ [Scheduler] Exception occurred. Retrying action ${action.id} in 1 min (Attempt ${retryCount + 1}/5)`,
            );
            payload.retryCount = retryCount + 1;
            try {
              await prisma.action.update({
        ...
        // Recurrence logic: trigger only on final state to ensure we don't break chains
        if (isFinalState && action.recurrence && originalTime) {
          try {
            ...
            if (nextDate) {
              console.log(
                `🔄 [Scheduler] Recurring action detected. Scheduling next instance for ${nextDate.toISOString()}`,
              );

              // Clean payload for the new instance
              const newPayload = { ...payload };
              delete newPayload.retryCount;
              delete newPayload.originalScheduledFor;
              ...
              await prisma.action.create({
                data: {
                  ...
                  status: "pending",
                  payload:
                    Object.keys(newPayload).length > 0 ? newPayload : undefined,
              ...
              console.warn(
                `⚠️ [Scheduler] Could not calculate next recurrence for action ${action.id} with rule: ${action.recurrence}`,
              );
            }
          } catch (recurErr) {
            console.error(
              `⏰ [Scheduler] Failed to schedule next recurrence for ${action.id}:`,
              recurErr,
            );
          }
        }
...
function calculateNextRecurrence(currentDate: Date, rule: string): Date | null {
  ...
  const applyIncrement = (d: Date) => {
    if (normalized === "daily" || normalized === "every day") {
      ...
      normalized.match(/every\s+(\d+)\s+(minute|hour|day|week|month|year)s?/)
    ) {
